...
class YaUploader:
    ...
    def upload_fails(self, file_path: str):
    ...
    def upload_file_to_disk(self, file_path, filename):
        href = self.upload_fails(file_path=file_path).get("href", "")
        response = requests.put(href, data=open(filename, 'rb'))
        response.raise_for_status()
        if response.status_code == 201:
            print("Success")
...
if __name__ == '__main__':
    ...
    yu = YaUploader(token=TOKEN)
    result = yu.upload_file_to_disk("dz", 'fails.txt')
    pprint.pp(result)
